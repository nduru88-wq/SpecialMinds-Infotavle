...
/***** MINI NÆSTE UGE - ÆGTE KLON AF HOVEDTAVLEN *****/

function visMiniNaesteUge() {
  var box = $("miniNaesteUge");
  var hovedUge = $("week");

  if (!box || !hovedUge) return;

  box.innerHTML = "";

  /*
    Miniaturen bygges nu med PRÆCIS de samme .week, .day og .event
    elementer som hovedtavlen. Derefter skaleres hele tavlen samlet ned.
    På den måde følger billeder, ikoner, noter, fødselsdage, farver osv.
    automatisk hovedtavlens layout.
  */
  var viewport = document.createElement("div");
  viewport.className = "mini-clone-viewport";

  var klonUge = document.createElement("div");
  klonUge.className = "week mini-clone-week";

  /* Brug hovedtavlens faktiske størrelse som miniature-tavlens lærred. */
  var originalBredde = Math.max(1, hovedUge.clientWidth);
  var originalHoejde = Math.max(1, hovedUge.clientHeight);

  klonUge.style.width = originalBredde + "px";
  klonUge.style.height = originalHoejde + "px";
  ...
  for (var i = 0; i < 5; i++) {
    ...
    day.innerHTML =
      "<h2>" + hentDagNavn(dato) +
      "<br><span>" + formatKortDato(dato) + "</span></h2>";
    ...
    klonUge.appendChild(day);
  }

  viewport.appendChild(klonUge);
  box.appendChild(viewport);

  function skalerMiniKlon() {
    var tilgaengeligBredde = box.clientWidth;
    var tilgaengeligHoejde = box.clientHeight;

    if (!tilgaengeligBredde || !tilgaengeligHoejde) return;

    var scale = Math.min(
      tilgaengeligBredde / originalBredde,
      tilgaengeligHoejde / originalHoejde
    );

    klonUge.style.transform = "scale(" + scale + ")";

    /* Centrér klonen hvis boksens proportioner ikke matcher hovedtavlen helt. */
    viewport.style.width = (originalBredde * scale) + "px";
    viewport.style.height = (originalHoejde * scale) + "px";
  }

  requestAnimationFrame(function() {
    skalerMiniKlon();

    /* tilpasEventHoejder() rammer nu både hovedtavlen og klonen.
       Begge har samme naturlige størrelse, så de får samme beregning. */
    setTimeout(function() {
      tilpasEventHoejder();
      skalerMiniKlon();
    }, 60);
